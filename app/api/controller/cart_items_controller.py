...
def get_cart_item_service(db: Session = Depends(get_db)) -> Any:
    repo = CartItemRepository(db)
    return CartItemService(repo)


@router.get("/", response_model=list[CartItemRead])
def list_cart_items(service: CartItemService = Depends(get_cart_item_service)) -> Any:
    return service.list_cart_items()


@router.get("/{item_id}", response_model=CartItemRead)
def get_cart_item(item_id: int, service: CartItemService = Depends(get_cart_item_service)) -> Any:
    return service.get_cart_item(item_id)


@router.post("/", response_model=CartItemRead, status_code=status.HTTP_201_CREATED)
def create_cart_item(payload: CartItemCreate, service: CartItemService = Depends(get_cart_item_service)) -> Any:
...
def delete_cart_item(item_id: int, service: CartItemService = Depends(get_cart_item_service)) -> None:
