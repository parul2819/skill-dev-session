...
def list_menu_items(service: MenuItemService = Depends(get_menu_item_service)) -> list[MenuItemRead]:
    return service.list_menu_items()


@router.get("/{item_id}", response_model=MenuItemRead)
def get_menu_item(item_id: int, service: MenuItemService = Depends(get_menu_item_service)) -> MenuItemRead:
...
def delete_menu_item(item_id: int, service: MenuItemService = Depends(get_menu_item_service)) -> Response:
