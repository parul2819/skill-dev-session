...
def list_order_items(service: OrderItemService = Depends(get_order_item_service)):
    return service.list_order_items()

@router.get("/{order_item_id}", response_model=OrderItemRead)
def get_order_item(order_item_id: int, service: OrderItemService = Depends(get_order_item_service)):
    return service.get_order_item(order_item_id)

@router.get("/order/{order_id}", response_model=list[OrderItemRead])
def list_order_items_by_order(order_id: int, service: OrderItemService = Depends(get_order_item_service)):
...
def create_order_item(payload: OrderItemCreate, service: OrderItemService = Depends(get_order_item_service)):
    return service.create_order_item(payload)

@router.patch("/{order_item_id}", response_model=OrderItemRead)
def update_order_item(order_item_id: int, payload: OrderItemUpdate, service: OrderItemService = Depends(get_order_item_service)):
    return service.update_order_item(order_item_id, payload)

@router.delete("/{order_item_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_order_item(order_item_id: int, service: OrderItemService = Depends(get_order_item_service)):
