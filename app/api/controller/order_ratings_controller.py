...
def get_order_rating_service(db: Session = Depends(get_db)) -> Any:
    repo = OrderRatingRepository(db)
    return OrderRatingService(repo)


@router.get("/", response_model=list[OrderRatingRead])
def list_ratings(service: OrderRatingService = Depends(get_order_rating_service)) -> Any:
    return service.list_ratings()


@router.get("/{rating_id}", response_model=OrderRatingRead)
def get_rating(rating_id: int, service: OrderRatingService = Depends(get_order_rating_service)) -> Any:
    return service.get_rating(rating_id)


@router.post("/", response_model=OrderRatingRead, status_code=status.HTTP_201_CREATED)
def create_rating(payload: OrderRatingCreate, service: OrderRatingService = Depends(get_order_rating_service)) -> Any:
