...
def get_user_address_service(db: Session = Depends(get_db)) -> Any:
    repo = UserAddressRepository(db)
    return UserAddressService(repo)


@router.get("/", response_model=list[UserAddressRead])
def list_addresses(service: UserAddressService = Depends(get_user_address_service)) -> Any:
    return service.list_addresses()


@router.get("/{address_id}", response_model=UserAddressRead)
def get_address(address_id: int, service: UserAddressService = Depends(get_user_address_service)) -> Any:
    return service.get_address(address_id)


@router.get("/user/{user_id}", response_model=list[UserAddressRead])
def list_user_addresses(user_id: int, service: UserAddressService = Depends(get_user_address_service)) -> Any:
    return service.list_user_addresses(user_id)


@router.post("/", response_model=UserAddressRead, status_code=status.HTTP_201_CREATED)
def create_address(payload: UserAddressCreate, service: UserAddressService = Depends(get_user_address_service)) -> Any:
